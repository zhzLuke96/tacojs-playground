...
export const App = (props?, children?) => {
  ...
  return html`
    <div ref=${[styleRef]}>
      <header>
        <${TopBtn}><${Icon} name="whatshot" /><//>
        <${TopBtn}>File<//>
        <${TopBtn}>Edit<//>
        <${TopBtn} color=${"rgb(45, 181, 93)"} onclick=${refrash}>Run<//>
        <${TopBtn}>Help<//>
        <${TopBtn} onclick=${() => window.open('https://github.com/zhzLuke96/tacojs-playground', '_blank')}><${GithubBtn} /><//>
      </header>
      <div>
        <${Editor}
          saveHandler=${refrash}
